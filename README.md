# LikeBot

Userbot для Telegram: входит в ваш аккаунт и ставит реакцию на каждое новое сообщение в выбранных чатах. Запускается в Docker.

Обычный бот из [@BotFather](https://t.me/BotFather) не может ставить лайки **от вашего имени**. Поэтому здесь используется клиент [Telethon](https://docs.telethon.dev): бот работает как вы, а не как отдельный бот-аккаунт.

## Что нужно заранее

1. Аккаунт Telegram.
2. `api_id` и `api_hash` с [my.telegram.org](https://my.telegram.org) → **API development tools**.
3. Docker и Docker Compose.
4. Id чатов, в которых нужно ставить лайки.
5. Бот из [@BotFather](https://t.me/BotFather), чтобы командой `/like` включать и выключать лайки.

## Быстрый старт

1. Скопируйте пример окружения и заполните свои значения:

```bash
cp .env.example .env
```

2. В `.env` укажите ключи API и чаты:

```env
TELEGRAM_API_ID=12345678
TELEGRAM_API_HASH=your_api_hash
CHAT_IDS=-1001234567890,-1009876543210
REACTION=heart
BOT_TOKEN=123456789:AAHxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```

3. Первый запуск должен быть интерактивным: Telegram пришлёт код подтверждения.

```bash
docker compose run --rm likebot
```

Введите номер телефона в международном формате (`+79001234567`), код из Telegram и пароль 2FA, если он включён. После успешного входа сессия сохранится в `./data`.

4. Дальше запускайте в фоне:

```bash
docker compose up -d --build
```

Логи:

```bash
docker compose logs -f likebot
```

Остановка:

```bash
docker compose down
```

## Логи

Смотреть логи в реальном времени:

```bash
docker compose logs -f likebot
```

Последние 100 строк:

```bash
docker compose logs --tail 100 likebot
```

Если контейнер назван иначе:

```bash
docker ps
docker logs -f likebot
```

При старте в логе должна быть строка `Реакция: ❤ [U+2764]`. Если Telegram отклоняет эмодзи, будет `Telegram отклонил лайк` или `Реакция ... недоступна`. Если задан `BOT_TOKEN`, ещё будет `Контрольный бот запущен: @ваш_бот`.

## Как узнать id чата

Самый простой способ — один раз запустить бота в режиме списка диалогов.

В `.env`:

```env
LIST_CHATS=true
```

Затем:

```bash
docker compose run --rm likebot
```

В консоли появятся строки вида:

```text
-1001234567890    Название чата
777000            Telegram
```

Скопируйте нужные id в `CHAT_IDS` через запятую и верните `LIST_CHATS=false`.

Также можно указать публичный `@username` чата.

Для супергрупп и каналов id почти всегда начинается с `-100`.

## Управление лайками через бота

Лайки ставит ваш аккаунт. Отдельный бот из BotFather нужен только как выключатель.

1. Откройте [@BotFather](https://t.me/BotFather) и отправьте `/newbot`.
2. Задайте имя и username, например `like_switch_bot`.
3. Скопируйте токен в `.env`:

```env
BOT_TOKEN=123456789:AAHxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```

4. По желанию в BotFather отправьте `/setcommands` и добавьте:

```text
like - Включить или выключить лайки
start - Показать текущий статус
```

5. Перезапустите контейнер и напишите своему боту в личку `/like`.

Команда `/like` переключает режим: включено ↔ выключено. Состояние сохраняется в `./data/likes_enabled` и не сбрасывается после рестарта. Команду принимает только тот аккаунт, под которым вошёл userbot.

## Фильтры по людям

В `.env` можно исключить человека или ставить ему другую реакцию:

```env
SKIP_USERS=Иван,@skip_user
POOP_USERS=Пётр,76543210
POOP_REACTION=poop
```

- `SKIP_USERS` — этим людям реакции не ставятся.
- `POOP_USERS` — вместо сердечка ставится 💩.
- Можно писать имя как в Telegram, `@username` или числовой id. Несколько человек — через запятую.

Имя лучше скопировать точно. Если не уверены, поставьте `LIST_USERS=true`, перезапустите контейнер и посмотрите список авторов:

```text
Иван Иванов @ivan (id=123456789)
```

Потом верните `LIST_USERS=false`. Надёжнее всего указывать id.

## Переменные окружения

| Переменная | Обязательная | Описание |
|---|---|---|
| `TELEGRAM_API_ID` | да | Числовой api_id с my.telegram.org |
| `TELEGRAM_API_HASH` | да | api_hash с my.telegram.org |
| `CHAT_IDS` | да | Чаты через запятую: `-100...` или `@username` |
| `REACTION` | нет | Реакция. Для сердечка пишите `heart`. Можно `❤`, `👍` или `like` |
| `SKIP_USERS` | нет | Кому не ставить реакции: имя, `@username` или id |
| `POOP_USERS` | нет | Кому ставить 💩 вместо лайка |
| `POOP_REACTION` | нет | Реакция для `POOP_USERS`. По умолчанию `poop` → 💩 |
| `LIKE_OWN_MESSAGES` | нет | `true`, если нужно лайкать и свои сообщения |
| `DELAY_SECONDS` | нет | Пауза перед реакцией. По умолчанию `0.4` |
| `BACKFILL_LIMIT` | нет | Сколько последних сообщений лайкнуть при старте. `0` — только новые |
| `SESSION_NAME` | нет | Имя файла сессии в `./data` |
| `LIST_CHATS` | нет | `true` — вывести список чатов и выйти |
| `LIST_USERS` | нет | `true` — вывести авторов сообщений и выйти |
| `TELEGRAM_SESSION_STRING` | нет | Готовая StringSession вместо файла |
| `BOT_TOKEN` | да, для /like | Токен бота от @BotFather |
| `LOG_LEVEL` | нет | Уровень логов: `INFO`, `DEBUG` |

Файл `.env` не коммитится. В репозиторий кладите только `.env.example`.

## Структура

```text
.
├── bot.py
├── Dockerfile
├── docker-compose.yml
├── requirements.txt
├── .env.example
└── .gitignore
```

Сессия Telegram монтируется в `./data`. Не публикуйте эту папку и не коммитьте `*.session`: по файлу сессии можно войти в аккаунт без кода из SMS.

## Запуск без Docker

```bash
python -m venv .venv
source .venv/bin/activate        # Windows: .venv\Scripts\activate
pip install -r requirements.txt
python bot.py
```

Сессию по умолчанию бот пишет в `/data`. Локально задайте другую папку:

```env
SESSION_DIR=./data
```

## Безопасность

- Храните `.env` и `./data` только у себя.
- Не включайте `*.session` и `.env` в git — они уже в `.gitignore`.
- Если сессия утекла, завершите все сеансы в Telegram: **Настройки → Устройства**.
- Ставьте умеренный `DELAY_SECONDS`. Слишком частые реакции Telegram может временно ограничить.

## Как это работает

Бот авторизуется вашим аккаунтом, подписывается на новые сообщения в чатах из `CHAT_IDS` и ставит выбранную реакцию. Свои сообщения по умолчанию пропускаются. Если задать `BACKFILL_LIMIT`, при старте он также пройдётся по последним сообщениям в каждом чате. Команда `/like` в личке с ботом из BotFather включает и выключает этот режим.

Telegram принимает сердечко именно как `❤` (`U+2764`). Вариант `❤️` часто даёт ошибку. Поэтому в `.env` надёжнее писать `REACTION=heart`.
